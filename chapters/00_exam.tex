...
\begin{description}
  \item[Author-date (APA)]
    Author format: \texttt{Last, Initial.} (e.g.\ \texttt{Uriol Balbin, I., Bisagni, C., \ldots}).
    Year in parentheses after authors: \texttt{(2020).}
    List ordered \textbf{alphabetically}.
    In-text: \texttt{(Author, Year)} or \texttt{Author (Year)}.

  \item[Numbered (AIAA)]
    Bracketed number before entry: \texttt{[1]}.
    Author format: \texttt{Last, Initial.}
    Entry: \texttt{[1] Last, I. "Title," Journal, Vol. X, No. Y, Year, pp. X–Y.}
    List ordered by \textbf{order of appearance} in text.
    In-text: \texttt{[1]}.
\end{description}
